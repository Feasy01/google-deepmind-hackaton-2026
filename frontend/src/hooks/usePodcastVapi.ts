        ...
        for (const toolCall of msg.toolCalls || []) {
          ...
          if (fnName === 'stop_player') {
            ...
            vapi.send({
              ...
              message: {
                role: 'system',
                content: `Podcast paused at ${timeStr} (${Math.floor(seconds)} seconds). Use this timestamp when calling ask_podcast.`,
              },
            })
...
IMPORTANT RULES:
1. When the user asks ANY question or says something that sounds like a question (e.g., "hey andrew, what is HRV?", "what did he mean by that?"), IMMEDIATELY call the stop_player tool to pause the podcast, then call ask_podcast with their question and the timestamp from the system message.
2. When the user says something like "thank you", "thanks", "got it", "resume", "continue", "play", or any dismissal phrase, call start_player to resume the podcast.
...
5. The podcast_id for this session is: ${podcastId}
6. Always resume the podcast, even after invalid tool use.
`,
          },
        ],
        tools: [
          ...
          },
          {
            ...
            function: {
              name: 'ask_podcast',
              description:
                'Ask a question about the podcast content. Uses RAG to find relevant context from the transcript near the current playback position.',
              parameters: {
                type: 'object',
                properties: {
                  question: {
                  ...
                  timestamp_seconds: {
                    type: 'integer',
                    description:
                      'The podcast playback position in seconds when the user asked the question. Get this from the system message injected after pausing.',
                  },
                },
